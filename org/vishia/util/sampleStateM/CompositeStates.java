...
public class CompositeStates  extends EventConsumer {
  ...
  private static class MainState extends StateTopBase<EMainState, CompositeStates>{

    protected MainState(CompositeStates env) {
    ...
    @Override public int switchState(Event ev) {
      ...
      switch(stateNr()){
        ...
        case Idle: cont = Idle.process(ev); break;
    ...
    private static class Idle extends StateBase<MainState, EMainState, CompositeStates>{
      
      
      Idle(MainState superState) { super(superState, EMainState.Idle); }
      ...
      @Override public int trans(Event ev) {
        if(ev instanceof EvX && ((EvX)ev).getCmd() == EvX.Cmd.EvX){
          int cont = superState.Work.C.C1.C1a.entry(consumed);
          cont |= superState.Work.C.C1.C1a.entry(consumed);
          return cont;
        }
    ...
    private static class Work extends StateComboBase<MainState, EMainState, EWork, CompositeStates>{

      protected Work(MainState superState) {
      ...
      private static class C extends StateComboBase<Work, EWork, EWork, CompositeStates> { //StateCombo<Work, EState_C1>{
        ...
        @Override public int trans(Event ev) {
          ...
          if(ev instanceof EvX && C1.stateNr() == EState_C1.C1b  && C2.stateNr() == EState_C2.C2b){
            exit();
            superState.B.entry(consumed);
            superState.B.B3.entry(consumed);
          }
          return StateBase.complete;
