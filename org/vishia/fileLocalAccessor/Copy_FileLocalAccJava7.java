...
public class Copy_FileLocalAccJava7
{
  ...
  final class EventCpy extends Event<CmdCpyIntern, CmdCpyIntern>{
     ...
     */
    EventCpy(FileAccessorLocalJava7 accessor){
    ...
    EventCpy(FileAccessorLocalJava7 accessor, boolean second){
  ...
  final FileAccessorLocalJava7 outer;
  ...
  Copy_FileLocalAccJava7(FileAccessorLocalJava7 accessor){
    this.outer = accessor;
  ...
  boolean processCheck(){
    ...
    if(backCmd != FileRemote.CallbackCmd.free){
      if(evBackInfo.occupyRecall(outer.evSrc, false)){
        ...
        evBackInfo.promilleCopiedBytes = (int)(((float)(Copy_FileLocalAccJava7.this.zBytesFileCopied) / Copy_FileLocalAccJava7.this.zBytesFile) * 1000);  //number between 0...1000
        if(zBytesAllCheck >0){
    ...
    @Override public int trans(Event<?,?> evP) {
      if(evP instanceof FileRemote.CmdEvent){
        ...
        if(cmdFile == FileRemote.Cmd.copyChecked || cmdFile == FileRemote.Cmd.delChecked || cmdFile == FileRemote.Cmd.moveChecked ){
          //gets and store data from the event:
          Copy_FileLocalAccJava7.this.modeCopyOper = ev.modeCopyOper;
          dirDst = ev.filedst;
    ...
    @Override public int trans(Event<?,?> ev) {
      ...
      if( (ret = transMoveFile(ev)) !=0) return ret;
        ...
        if(actData.dst.exists()){
          if(bOverwrfile){  //The last event was "overwrite", therefore overwrite only this one file.
          ...
          else if((Copy_FileLocalAccJava7.this.modeCopyOper & FileRemote.modeCopyExistMask) == FileRemote.modeCopyExistSkip){
            ...
            switch(Copy_FileLocalAccJava7.this.modeCopyOper & FileRemote.modeCopyExistMask){
            ...
            switch(Copy_FileLocalAccJava7.this.modeCopyOper & FileRemote.modeCopyReadOnlyMask){
        ...
        Copy_FileLocalAccJava7.this.zBytesFile = actData.src.length();
        Copy_FileLocalAccJava7.this.zBytesFileCopied = 0;
        try{ 
          FileSystem.mkDirPath(actData.dst);
          Copy_FileLocalAccJava7.this.out = new FileOutputStream(actData.dst);
        } catch(IOException exc){
          sendEventAsk(actData.dst, FileRemote.CallbackCmd.askErrorDstCreate );
          return exitState.stateAsk.entry(null);
        }

        try{ Copy_FileLocalAccJava7.this.in = new FileInputStream(actData.src);
        } catch(IOException exc){
  ...
  private class StateCopyFileContent extends StateSimpleBase<StateCopyProcess>{
    ...
    @Override public int trans(Event<?,?> evP) {
      ...
      if(ev.cmdi == CmdCpyIntern.file){
        ...
        do {
          try{
            int zBytes = Copy_FileLocalAccJava7.this.in.read(buffer);
            if(zBytes > 0){
              Copy_FileLocalAccJava7.this.zBytesFileCopied += zBytes;
              zBytesAllCopied += zBytes;
              Copy_FileLocalAccJava7.this.out.write(buffer, 0, zBytes);
    ...
    @Override protected void exitAction(){
      try{
        if(Copy_FileLocalAccJava7.this.in!=null){ 
          zFilesCopied +=1;
        ...
        Copy_FileLocalAccJava7.this.out = null;
  ...
  private class StateNextFile extends StateSimpleBase<StateCopyProcess>{
    ...
    @Override public void entryAction(Event<?,?> ev){
      ...
      if(Copy_FileLocalAccJava7.this.in !=null){
        zFilesCopied +=1;
        try{ Copy_FileLocalAccJava7.this.in.close();
        } catch(IOException exc){
          System.err.printf("FileAccessorLocalJava7 -Copy close src while abort, unexpected error; %s\n", exc.getMessage());
        }
        Copy_FileLocalAccJava7.this.in = null;
      }
      if(Copy_FileLocalAccJava7.this.out !=null){
        try{ Copy_FileLocalAccJava7.this.out.close();
        } catch(IOException exc){
